...
from tkinter import Toplevel, Label, Entry, Button, Radiobutton, IntVar
...
class FormularioMaestroDesign(tk.Tk):
    ...
    def controles_barra_superior(self):
        ...
        menu_gestion.add_command(label="Generar Reporte", command=self.generar_reporte)
    ...
    def crear_secciones(self):
        ...
        #Botón tickets
        self.boton_Tickets = tk.Button(self.menu_superior, text="Tickets Abiertos",
                                       command=lambda: self.mostrar_sub_seccion('tickets'))
        self.boton_Tickets.pack(side="left", padx=10, pady=10)
        #Botón Entradas-Salidas
        self.boton_entradas_salidas = tk.Button(self.menu_superior, text="Tickets Cerrados",
        ...
        self.label_total = tk.Label(self.sub_secciones["tickets"], text="Total: $0", font=("Arial", 14), bg="lightblue")
        self.label_total.pack(pady=10, anchor="w")
        ...
        self.tree_cancelado.heading("hora", text="Entrada")
    ...
    def mostrar_sub_seccion(self, sub_seccion):
        ...
        if sub_seccion in self.sub_secciones:
            ...
            if sub_seccion == 'tickets':
                ...
                self.label_imagen.pack_forget()  # Ocultar imagen en historial
    ...
    def mostrar_widgets_inicio(self):
        ...
        def actualizar_total(self):
            ...
            for child in self.tree.get_children():
                precio = self.tree.item(child, "values")[8]  # Índice 8 para la columna "precio"
                try:
                    total += int(precio)
                except ValueError:
                    print(f"Error en el valor de precio: {precio}")
    ...
    def liberar_espacio(self, folio_a_liberar, ventana_folio):
        ...
        if ticket:
            ...
            self.db_cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', ("Salida", ticket[2], ticket[3], ticket[4], ticket[5], ticket[6], ticket[7], ticket[8], hora_salida, ticket[9]))
            self.db_connection.commit()
            ...
            # Añadir el ticket actualizado a la subsección "salidas" (Treeview)
            self.tree_entradas.insert('', 'end', values=(
            ticket[1], ticket[2], ticket[3], ticket[4], ticket[5], ticket[6], ticket[7], ticket[8], hora_salida, ticket[9]))
    ...
    def cancelar_ticket(self, folio_a_liberar, ventana_folio):
        # Buscar el ticket en la base de datos
        self.db_cursor.execute("SELECT * FROM rentas WHERE folio = ?", (folio_a_liberar,))
        ticket = self.db_cursor.fetchone()
        ...
        if ticket:
            ...
            self.db_cursor.execute('''
                INSERT INTO salidas (estado_salida, folio, numero_placas, nombre_marca, modelo_vehiculo, color_coche, numero_telefono, hora_registro, hora_salida, precio_boleto)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', ("Salida", ticket[2], ticket[3], ticket[4], ticket[5], ticket[6], ticket[7], ticket[8], hora_salida,
                  ticket[9]))
            self.db_connection.commit()
            ...
            # Añadir el ticket actualizado a la subsección "salidas" (Treeview)
            self.tree_entradas.insert('', 'end', values=(
                ticket[1], ticket[2], ticket[3], ticket[4], ticket[5], ticket[6], ticket[7], ticket[8], hora_salida,
                ticket[9]))
    ...
    def actualizar_seccion_cancelados(self):
        ...
        for salida in salidas:
            # Insertar cada salida en el Treeview
            self.tree_entradas.insert('', 'end', values=(
                salida[1], salida[2], salida[3], salida[4], salida[5], salida[6], salida[7], salida[8], salida[9],
                salida[10]))

    def generar_reporte(self):
        print("Función para generar reporte ejecutada.")
